...
class DictController : UIViewController, UITextFieldDelegate {
    ...
    var words = [Word]()
    var timerForSearchingWords : Timer?
    ...
    let mostUsedWordsButton : UIButton = {
        ...
        button.clipsToBounds = true
        button.layer.cornerRadius = 20
        button.addTarget(self, action: #selector(handleOpenMostUsedWordsController), for: .touchUpInside)
        return button
    }()
    ...
    @objc private func textFieldDidChange() {
        ...
        timerForSearchingWords?.invalidate()
        timerForSearchingWords = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: false, block: { (_) in
    ...
    @objc private func handleOpenMostUsedWordsController() {
        
        
    }
    
    @objc private func  handleOpenYourWordsController() {
    ...
    var leftAnchorWithInset: NSLayoutConstraint? // тестировал анимацию констрейнтов. пока оставил для теста
    var rightAnchorWithInset: NSLayoutConstraint?
    
    private func setupUI() {
        
        view.backgroundColor = .white // не нужен но пусть будет для тестов
        view.addSubview(backGroundImage)
        
        //MARK: Setup table view delegate data source and cell
        ...
        backGroundImage.frame = view.frame // ставим фрейм чтобы при открытии не было видно фона
